...
import {
  users, courses, evaluations, lecturerCourses, evaluationPeriods,
  type User, type InsertUser, type Course, type Evaluation,
  type EvaluationPeriod, type EvaluationSummary
} from "@shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  async getLecturers(): Promise<(User & { courseCode?: string, courseName?: string, courseId?: number })[]> {
    ...
    return result.map(r => ({ ...r.user, courseId: r.course.id, courseCode: r.course.code, courseName: r.course.name }));
  }
  ...
  async getAllPeriods(): Promise<EvaluationPeriod[]> {
    return await db.select().from(evaluationPeriods).orderBy(evaluationPeriods.createdAt);
  }
  async createPeriod(data: { name: string; startDate: Date; endDate: Date; isActive: boolean }): Promise<EvaluationPeriod> {
    if (data.isActive) {
      await db.update(evaluationPeriods).set({ isActive: false });
    }
    const [period] = await db.insert(evaluationPeriods).values(data).returning();
    return period;
  }
  async deletePeriod(id: number): Promise<void> {
    await db.delete(evaluationPeriods).where(eq(evaluationPeriods.id, id));
  }
  async activatePeriod(id: number): Promise<EvaluationPeriod> {
    await db.update(evaluationPeriods).set({ isActive: false });
  ...
  async getActivePeriod(): Promise<EvaluationPeriod | undefined> {
    const [period] = await db.select().from(evaluationPeriods).where(eq(evaluationPeriods.isActive, true));
    return period;
  }
  ...
  async getLecturerSummary(lecturerId: number): Promise<EvaluationSummary & { courseBreakdowns: CourseSummary[] }> {
    ...
    const empty: EvaluationSummary & { courseBreakdowns: CourseSummary[] } = {
      ...
      courseBreakdowns: assignedCourses.map(course => ({
        ...
        averageOverall: 0, averageClarity: 0, averageEngagement: 0, averageMaterials: 0,
        averageOrganization: 0, averageFeedback: 0, averagePace: 0, averageSupport: 0,
        averageFairness: 0, averageRelevance: 0, comments: [],
      })),
    };
    ...
    const courseBreakdowns: CourseSummary[] = assignedCourses.map(course => {
      ...
      return {
        course, totalEvaluations: courseEvals.length,
        averageOverall: stats?.averageOverall ?? 0, averageClarity: stats?.averageClarity ?? 0,
        averageEngagement: stats?.averageEngagement ?? 0, averageMaterials: stats?.averageMaterials ?? 0,
        averageOrganization: stats?.averageOrganization ?? 0, averageFeedback: stats?.averageFeedback ?? 0,
        averagePace: stats?.averagePace ?? 0, averageSupport: stats?.averageSupport ?? 0,
        averageFairness: stats?.averageFairness ?? 0, averageRelevance: stats?.averageRelevance ?? 0,
