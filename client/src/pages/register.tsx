...
export default function Register() {
        ...
        <div className="flex items-center justify-center py-12">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
            <div className="bg-white rounded-3xl shadow-2xl shadow-indigo-100/50 p-10 border border-slate-100 text-center">
              ...
              <p className="text-slate-500 mb-2">We've sent a verification link to <span className="font-semibold text-slate-700">{email}</span>.</p>
              <p className="text-sm text-slate-400 mb-6">Click the link in the email to verify your account.</p>
              <Button
  ...
  const isSubmitDisabled = isRegistering || isLoadingCourses || !(isLecturerValid || isStudentValid);
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <Input id="name" value={name} onChange={e => setName(e.target.value)} required className="px-4 py-5 rounded-xl bg-slate-50/50" placeholder="Jane Doe" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="username">Username</Label>
                  <Input id="username" value={username} onChange={e => setUsername(e.target.value)} required className="px-4 py-5 rounded-xl bg-slate-50/50" placeholder="janedoe" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} required className="pl-10 py-5 rounded-xl bg-slate-50/50" placeholder="you@example.com" />
                </div>
                <p className="text-xs text-slate-400 flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Used for account verification and password reset
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="password">Password</Label>
                <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={8} className="px-4 py-5 rounded-xl bg-slate-50/50" placeholder="Min. 8 characters" />
              </div>
              ...
              {/* LECTURER: department + multiple courses */}
              {role === 'lecturer' && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-5 pt-2 border-t border-slate-100">
                  <div className="space-y-2">
